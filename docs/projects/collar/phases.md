...
- **2026-09-07 — Collar phase 3's stop debt is paid: a zombie counts as gone.** `alive()` reads `/proc/<pid>/stat` or `ps -o stat=`, the post-SIGKILL wait is three seconds, then `unreaped` is reported; a test makes a zombie with `exec sleep`.
