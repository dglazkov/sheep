...
- **2026-09-07 — The image by digest is the workflow's to know.** `docker push` prints the digest and the release runs after it, so the shipped config and `SHEEP_IMAGE` name `…@sha256:`; a candidate built here names the tag, and the ring's three-way check, Worker, config, application, held on the tag.
...
- **2026-09-07 — The ring held thirteen lines: deploy 46 s, the second machine's container 28 s, 3824 `ps` samples clean.** Cost: 20 minutes of a subagent, 40 of verification.
...
**Status: PART-DONE.** 2026-09-07. `sheep home join`, the second machine in the account ring, journey 2 walked in the ring and on `sheep-2` with a real model; journey 3 built into the ring and skipped without the repository's token, which the Open finding names.
