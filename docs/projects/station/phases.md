...
- **2026-09-07 — Open: the checkout-deployed `sheep` and `sheep-pen`.** Deleted when the shepherd says so, by name, with `sheep home delete --name` from a kennel holding the token; the name `sheep` is then free for a station.

**Status: PART-DONE.** 2026-09-07. Journey 4 walked in the ring and on `sheep-2`; journey 5 whole but for journey 3's half, which waits on the repository's token (station phase 2's Open finding); the checkout homes' deletion waits on the shepherd's word.
