...
- **2026-09-07 — The checkout's `sheep` and `sheep-pen` deleted on the shepherd's word,** by name from a kennel holding the token but not theirs, so the listing counted their sessions as unknown; the account holds `sheep-2` alone, and the name `sheep` is free.
