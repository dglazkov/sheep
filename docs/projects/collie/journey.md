...
since: 2026-09-13
see: collie
note: "planned 13 Sep 2026 from the shepherd's issue #12: isocan's rc, the mechanical dog that answers a canvas's summonses by prompting sheep, hosted so no laptop stays open. Planned first as `sheep collie …`, recut into a repository of its own when the shepherd asked whether the collie was its own CLI, and moved back the same evening once it read as a complement built from sheep's parts: `collie` is a second command in this package, with a Worker of its own on the shepherd's account, one Durable Object that runs isocan's rc awake, holding isocan's long polls and prompting sheep over the sheep home's routes. Isocan's rc becomes a module with no Node in it (the brain, isocan's); the sheep home and the `sheep` command are untouched. Four phases, the first in isocan: closed there on 14 Sep 2026 as its project `room` (isocan#294), verified from a scratch install. The second closed the same day: isocan 14f39ba5 exported its route client to the no-Node entry, `packages/collie` hosts the room in a Durable Object held in workerd against scripted fakes, `collie` reports, logs, switches, and takes a pass at a hidden prompt, and the rig stood by by hand. The third is built and its home ring walks journey 1 on the rig against a real isocan daemon, after isocan 8729b9e3 made a stopped rc stop answering at once; its package ring held, and the account's collie-only walk held on release be3d642: setup, the mint, the ask, a real model's reply on dev.isocan.io with no laptop rc, the switch, and the end. The fourth closed the same day: `collie pass --agent` hands a laptop rc's agent to the collie, which resumes the agent's existing sheep, walked in the home ring beside a real laptop `isocan rc`; isocan gained `isocan pass --agent` (e4bc100f). The collie is built; two steps wait on isocan: off within a second on a hosted home (isocan#308) and `isocan who` on a replica (isocan#306)."
---
...
- A collie speaks as an agent only when its badge holds that agent's
  ...
  narration, in isocan's words — `Shaun is not held by this machine — a
  pass from whoever holds Shaun hands it over` — and never claims, never faces, never fails a turn
  for.
