...
**Where we are: earmark is done, 11 Sep 2026. Both phases CLOSED.** A
dog can give one sheep a secret of its own: `sheep new --secret <NAME>`,
the value a line of stdin, rows beside the sheep's in the Directory,
laid over its pasture's in setup and in the broker, named in `sheep ls`,
ended with the sheep. Planned and built in a day from the shepherd's
issue #5. Phase 0 proved the mechanism in workerd; phase 1 added the
verb, the names, the docs, and journey 5 in the home ring, walked
journey 1 on the local home with Docker and a real model, and closed on
the account ring the shepherd typed, `ok s1` on the station. Three open
debts under phase 1; nothing waits on work.
...
**Status: CLOSED.** 2026-09-11. Journeys 1 to 4 walked: 1 and 3 on the local home and in the home ring, 1 with Docker and a real model, each sheep's setup hashing its own value; 2 steps 1 and 2 and 4 step 2 by the account ring the shepherd typed, `ok s1` on release 0afc695; `pnpm test` exits 0 across all three rings.

**Findings:**

- **2026-09-11 — The values are read in `main`, not `dispatch`:** `dispatch` runs twice when the local home starts on demand, by when stdin is spent; every refusal is before the first request.
- **2026-09-11 — The walk: a mint with a secret in 0.18 s, no container;** each first prompt cloned and set up in 2.4 s; the earmarked sheep's setup hashed its own `PROBE`, the sibling's the pasture's, both as computed; `env` counted 0 in each.
- **2026-09-11 — The guide paid for its `--secret` bullet with cut redundancies:** 1497 words against the cap of 1500.
- **2026-09-11 — The account ring on d37f9e6 failed at `s1` on the container cap, not the earmark:** beside a8 the sibling could rent no container (`max_instances` 3, idle 10 min). `s1` now runs after n1 and ends its own two.
- **2026-09-11 — The account ring on 0afc695, typed by the shepherd: `ok s1`.** `own` pushed with its own token in 15 s; the sibling refused, "could not read Username"; the token nowhere; the delete listed 0 sessions, 9 minted and 9 ended.
- **2026-09-11 — Open: the broker's refusal reaches git as nothing;** the sheep reads git's line, never the sentence naming sheep, pasture, and home. Pen's to change.
- **2026-09-11 — Open: `join.test.ts`'s terminal case passes without running on macOS:** `script` will not start with a socket as stdin.
- **2026-09-11 — Open: a7's join from a fresh container once found the station silent:** `whoAnswers` gives up at 2 s, and a cold container's first lookup took longer; the rerun passed.
