...
since: 2026-09-09
see: serve
note: "written 9 Sep 2026, from a conversation with the shepherd: a sheep building a frontend app needs its dev server, and a standing server would occupy the container, so the server is rented for the length of a look. The shepherd's calls: `look --serve` in the shell, one flag on the program the sheep has; the server an ordinary run, killed when the look is done, a server kept across a turn the named debt; the forward as two frames on the container's one socket, so the container stays a client of the cell and the proof runs in workerd through the fake, the platform's port binding left; the recipe for a frontend app a pasture skill, shipped in the project; no script in the image. Phase 0 closed 10 Sep 2026: the forward is built and proved in workerd — `fetch` and `response` on the container's one socket, the agent's fetcher, the fake's stood ports, the cell's third reader, and the eyes rendering from an `Origin` the rows are now one maker of. Journey 4 holds whole. Phase 1 closed the same day: the flags, the rental, the `server` section and the prompt's sentences, and journey 1 walked on the local home with a real model — which failed on its own first line and changed the design twice, since a server on `localhost` in the image binds `::1` and the closing line was reading two clocks. Journey 1 holds but for one criterion: the second served look takes 2.5 s, not under two. Phase 2 closed the same day: the rings walk a served look in a real container and read the kill from `/proc` inside it, the words say a sheep can look at its dev server, and the frontend skill ships here. All four journeys are walked — 1 and 2 on the local home with a real model, 3 on the shepherd's account with `sheep-2` upgraded and the account ring green, 4 in workerd. The walks changed the design three times: the forward reaches either loopback, the closing line reads one clock, and the ring waits for the platform's rollout before it looks."
---
