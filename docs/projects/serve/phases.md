...
- **2026-09-10 — The ring waits on the platform's own signal, not a clock and not a retry.** `s2` waits for the rollout targeting the station's image to read `completed` and the application to be configured with it, `deploy.ts`'s own two conditions; a look that fails after it still fails the ring. Corrected 11 Sep 2026: the second condition is gone from both, since the platform mirrors the image into its own registry and no configuration of it can name what we deployed.
- **2026-09-11 — `s2` failed on release 72b6ac6 waiting for a condition that cannot come true.** The rollout to the deployed image completed at 65 s and the application stayed configured with the older one for the whole 300 s budget; the wait now ends on the rollout's own word, and what the configuration names is said beside it.
