...
since: 2026-09-14
see: hill
note: "planned 14 Sep 2026, the day drove closed, from the shepherd's ask for an observation point: a Web UI they can sign into easily and watch sheep in action, starting very small (sign in, see status) and going deep, Chrome DevTools style, over later projects. The calls: the hill is a face of the station itself, at `/hill/`, never a second Worker; the way in is a one-time link `sheep hill` prints, exchanged for a cookie, so the home's token never reaches a browser; the hill only observes in this project. Four phases: the door, the shell of the page and its shipping, the flock, and one sheep live. Orienting for phase 0 found a seat would have reached the WebSocket, whose protocol writes, and the docs refused it first. Hill phase 0 closed the same day: a pass minted under the bearer buys one seat, used or expired after, the seat's HttpOnly cookie reads and is the bare 401 for any write and for an upgrade, sign out ends it, `sheep hill` prints the link or refuses an older home; proved in workerd and the command ring, three mutations falsified. Hill phase 1 is part-done the same day: the station serves the page at `/hill/` from an assets binding before the door, the gate for no seat and the shell once seated, the pass out of the address whatever the answer, and a release carrying `home/hill/` held the package ring on the phase's commit; the conductor walked journey 1 steps 1 to 3 and 5 in Chrome, and the phase closes on the shepherd's yes to the look. Hill phase 2 is part-done the same day: the flock, polled every two seconds while visible and never while hidden, walked on a local home with Docker and a real model, each dog's command on the page within two seconds and the setup's words the same as `sheep status`'s; ids are inked by their last eight, since a uuidv7's first eight are shared by sheep minted in the same minute; step 6's hour was begun. Hill phase 3 is part-done on 15 Sep: `sheep log`'s rendering moved into blocks the page draws, byte for byte what it printed, a sheep's page following its transcript over the long poll and never waking a sheep nothing has asked, and the seat's cookie named for its home after two local homes signed each other out; its walk was walked the same morning on a station deployed for it and deleted after, journeys 1 to 3 in a headless Chrome with a real model, and journey 2 step 6's hour held there, 1,798 asks in 3,601 s; the account ring failed three times before `h1`, at three different steps and never at a hill one, whose checks held by hand on a deployed station. The shepherd said yes to the look on 16 Sep 2026, closing hill phases 1 and 2; hill phase 3 waits on an account ring that reaches `h1`."
---
