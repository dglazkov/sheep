...
since: 2026-09-13
see: shear
note: "written 13 Sep 2026 after the shepherd asked for the upgrade story and liked codex's shape: `npm install -g` and the old copy says it is out of date. Sheep takes that shape for the command and adds the half codex lacks, the home. A command hears once from the release branch's tip that a newer build is out; the home says its build on every response, so any command says once that one side is older; `sheep home deploy` refuses while sheep are mid-turn unless told `--now`; and a verb a too-old home cannot answer is refused with the sentence that fixes it, by a floor the command carries. The skew rule stops calling two builds of one commit skew. Nothing is fetched, installed, or deployed on its own. Two phases, both closed 13 Sep 2026. Shear phase 0, the notice and the header: its walk on its release found the tip's fetch in the command's own process never landed for a fast verb, so the fetch moved to a detached child, walked again on release 6261bfc. Shear phase 1, the floor and the guard: the floor reads a 4xx other than 401, since a thrown route's 500 carries no header; the account ring upgraded 6261bfc to e75eda3 on the shepherd's account, the notice read from GitHub's tip, the skew line once, a sheep mid-turn refusing the deploy twice, `--now` reporting one interrupted, the held wait returning, and the station deleted. Done."
---
