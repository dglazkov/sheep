...
- **2026-09-11 — The account ring's first run failed at `s2`, before `b1`.** Not bleat's: the platform had begun mirroring container images into its own registry, so the settle waited on a configuration that could never name the image; fixed in `7d3b22d` under station and serve.
- **2026-09-11 — Open: journey 4 step 3 waits on the shepherd.** `pnpm hermetic --ring account --yes <sha>` with `b1` deploys and deletes a station on their account; the run also proves `7d3b22d`. Nothing else in bleat waits on work or on a person.
