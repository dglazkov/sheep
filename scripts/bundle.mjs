...
export const EYES_BINDING = "BROWSER";

/**
 * The eyes ride the config (eyes phase 2): a binding is not inherited into
 * a named environment, so the shipped config must bind `BROWSER` at the
 * top level (the local home, `wrangler dev` over it) and in `env.pen` (the
 * station), or a release would deploy a home whose every sheep is told it
 * has no eyes. Checked after `shippedConfig`, since that is what is
 * written; returns the config it was given.
 */
export function assertEyes(config) {
  const missing = [
    ["the top level", config?.browser],
    ["env.pen", config?.env?.pen?.browser],
  ]
    .filter(([, browser]) => browser?.binding !== EYES_BINDING)
    .map(([where]) => where);
  if (missing.length > 0) throw new Error(`the shipped config has no browser binding named ${EYES_BINDING} at ${missing.join(" or ")}; the home it deploys would have no eyes`);
  return config;
}

/** The binding the join store is read through (stile phase 2): `env.JOIN`, the station's `<worker>-join` KV namespace. */
export const JOIN_BINDING = "JOIN";

/**
 * The join rides the config (stile phase 2): the station's `pen`
 * environment must bind `JOIN` as a KV namespace with no id, which `sheep
 * home deploy` fills in with the namespace it makes, or a joining machine
 * would write a key no Worker reads and wait ninety seconds for nothing.
 * The top level carries it too, for the local home. Returns the config.
 */
export function assertJoin(config) {
  const bound = (place) => Array.isArray(place?.kv_namespaces) && place.kv_namespaces.some((namespace) => namespace?.binding === JOIN_BINDING && namespace.id === undefined);
  const missing = [
    ...
    .map(([where]) => where);
  if (missing.length > 0) throw new Error(`the shipped config has no id-less KV binding named ${JOIN_BINDING} at ${missing.join(" or ")}; a station it deploys could not be joined`);
  return config;
}
